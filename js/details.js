...
function openDetail(id){
  ...
  var tarSec='';
  if(isK){
    var ki=parseInt(p.kazna||0);var base=p.uloga==='osteceni'?getTKO(ki):TAR_K[ki];var mul=p.sld?0.5:1;
    tarSec='<div class="ds">Tarifa 2025 — T.br.1</div><div class="tt"><div class="tt-h">'+(p.uloga==='osteceni'?'Punomoćnik oštećenog':p.uloga==='osumnjiceni'?'Branilac osumnjičenog':'Branilac okrivljenog')+(p.sld?' · Po sl. dužnosti':'')+' · '+KL[ki]+'</div>'
      +'<div class="tt-r"><span class="tt-l">Odbrana na pretresu</span><span class="tt-v">'+fmt(Math.round(base.od*mul))+'</span></div>'
      +'<div class="tt-r"><span class="tt-l">Zastupl. oštećenog / neodržan</span><span class="tt-v">'+fmt(Math.round(base.zo*mul))+'</span></div>'
      +'<div class="tt-r"><span class="tt-l">Inicijalni akti</span><span class="tt-v">'+fmt(Math.round(base.ini*mul))+'</span></div>'
      +'<div class="tt-r"><span class="tt-l">Ostali podnesci</span><span class="tt-v">'+fmt(Math.round(base.ost*mul))+'</span></div>'
      +'<div class="tt-r"><span class="tt-l">Žalba na presudu</span><span class="tt-v hi">'+fmt(Math.round(base.zal*mul))+'</span></div></div>';
  } else {
    var t4=(p.vrsta==='parnicni'||p.vrsta==='vanparnicni')?(p.neprocenjiv?TAR_N[parseInt(p.nproIdx||0)]:getTarP(p.vred)):p.vrsta==='izvrsni'?getTarI(p.vred):null;
    if(t4) tarSec='<div class="ds">Tarifa 2025</div><div class="tt"><div class="tt-h">'+VL[p.vrsta]+' · '+(p.neprocenjiv?('Neprocenjivi · '+TAR_N[parseInt(p.nproIdx||0)].l):((p.vred||0).toLocaleString('sr-RS')+' din'))+'</div><div class="tt-r"><span class="tt-l">Podnesak</span><span class="tt-v">'+fmt(t4.pod)+'</span></div><div class="tt-r"><span class="tt-l">Ročište</span><span class="tt-v">'+fmt(t4.roc)+'</span></div><div class="tt-r"><span class="tt-l">Neodržano</span><span class="tt-v">'+fmt(t4.neo)+'</span></div><div class="tt-r"><span class="tt-l">Žalba</span><span class="tt-v hi">'+fmt(t4.zal)+'</span></div></div>';
    if(p.vred&&p.tipSuda){var st2=getST(p.vred,p.tipSuda);if(st2)tarSec+='<div class="ds">Sudska taksa — '+SUDL[p.tipSuda]+'</div><div class="tt"><div class="tt-r"><span class="tt-l">Taksa za tužbu</span><span class="tt-v">'+fmt(st2.tuzba)+'</span></div><div class="tt-r"><span class="tt-l">Taksa za presudu</span><span class="tt-v">'+fmt(st2.presuda)+'</span></div><div class="tt-r"><span class="tt-l">Žalba</span><span class="tt-v hi">'+fmt(st2.zalba)+'</span></div></div>';}
  }
  ...
  document.getElementById('mo-dc').innerHTML=
    ...
    +(p.tuz2&&p.vrsta!=='tuzilastvo'?'<div class="tt-r"><span class="tt-l">'+lbl2+'</span><span class="tt-v">'+p.tuz2+'</span></div>':'')
