...
function savePot(){
  ...
  var obj={
    id:Date.now().toString(),pid:pid||'',
    br:document.getElementById('pot-br').value.trim()||br,
    klijent:klijent,iznos:iznos,
    status:_potSt,
    dat:document.getElementById('pot-dat').value,
...
function openPotAct(id){
  ...
  document.getElementById('potact-info').innerHTML=
    '<div style="font-size:13px;color:var(--t2);margin-bottom:4px">Klijent: <b style="color:var(--tx)">'+pot.klijent+'</b></div>'
    +'<div class="pot-iznos">'+fmt(pot.iznos)+'</div>'
    +'<div style="margin-top:8px">'+potBdg(pot.status)+'</div>';
  var stMap={
  ...
  var actions=stMap[pot.status]||[];
  actions.push({l:'🗑 Obriši',fn:'potDel',cls:'btn-rd'});
  document.getElementById('potact-btns').innerHTML=actions.map(function(a){
    return '<button class="btn '+a.cls+'" style="width:100%;justify-content:center;padding:12px;font-size:14px" onclick="'+a.fn+'(\''+id+'\')">'+a.l+'</button>';
  }).join('');
...
function potBdg(st){
  ...
  return '<span class="pot-status '+(cls[st]||'bg')+'">'+( PST[st]||st)+'</span>';
}
function renderPot(){
  var act=D.pot.filter(function(x){return x.status!=='placeno';});
  var el=document.getElementById('lpot');
  document.getElementById('sn-pot').textContent=act.length;
  ...
  else el.innerHTML=act.map(function(pot){
    return '<div class="card co" onclick="openPotAct(\''+pot.id+'\')">'
      +'<div class="ch"><div class="cn">'+pot.br+'</div>'+potBdg(pot.status)+'</div>'
      +'<div class="cm">'+(pot.klijent?'<b>'+pot.klijent+'</b><br>':'')
      +(pot.dat?'Odluka: '+fmtDs(pot.dat)+'<br>':'')
      +(pot.nap?pot.nap:'')+'</div>'
      +'<div class="pot-iznos">'+fmt(pot.iznos)+'</div>'
      +'</div>';
  }).join('');
  var arh=D.arh;
  var arhDs=document.getElementById('arh-ds');
  var lArh=document.getElementById('larh');
  if(!arh.length){arhDs.style.display='none';lArh.innerHTML='';return;}
  arhDs.style.display='flex';
  lArh.innerHTML=arh.slice(0,10).map(function(pot){
    return '<div class="arh-card">'
      +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px">'
      +'<div style="font-family:\'Cormorant Garamond\',serif;font-size:15px;font-weight:700;color:var(--gr)">'+pot.br+'</div>'
      +'<div style="font-size:10px;color:var(--t3)">'+( pot.datPlacanja?fmtDs(pot.datPlacanja):'')+'</div>'
      +'</div>'
      +'<div style="font-size:12px;color:var(--t2)">'+(pot.klijent||'')+'</div>'
      +'<div class="pot-iznos plac" style="font-size:16px;margin-top:3px">'+fmt(pot.iznos)+'</div>'
      +'</div>';
  }).join('');
}
